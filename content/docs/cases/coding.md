...
{{< alert context="info" text="**想定シーン**: 大学職員が、2つのシートに分かれた学生データを個人IDで結合して一覧表を作りたいとき" />}}
...
{{< alert context="info" text="**想定シーン**: 大学職員が、成績データCSVからスコア80以上の学生を抽出し、ID順に並べたリストを作成したいとき" />}}
...
{{< alert context="info" text="**想定シーン**: 大学職員が、名簿データから敬称を一括削除して宛名リストを整備したいとき" />}}
...
{{< alert context="info" text="**想定シーン**: 大学職員が、ScopusのAPIから取得したJSON形式の研究者業績データをデータフレームに変換して分析したいとき" />}}
...
{{< alert context="info" text="**想定シーン**: 大学職員が、名簿の敬称削除をVBAで自動化し、処理件数のログも残したいとき" />}}
...
Sub RemoveTitle()
    ...
    For i = 2 To lastRow
        ...
        name = Replace(Replace(name, "様", ""), "殿", "")
        ws.Cells(i, 2).Value = Trim(name)
        count = count + 1
    Next i

    Debug.Print "処理件数: " & count & "件"
End Sub
```

**使い方**: VBAエディタ（Alt+F11）から標準モジュールに貼り付けて実行します。イミディエイトウィンドウ（Ctrl+G）で処理件数を確認できます。
{{% /alert %}}
...
{{< alert context="info" text="**想定シーン**: 大学職員が、前任者から引き継いだPythonスクリプトの処理内容を理解するためにコメントを追加したいとき" />}}
...
{{< alert context="info" text="**想定シーン**: 大学職員が、大学Webサイトのナビゲーション部分のJavaScriptコードを理解したいとき" />}}
...
{{< alert context="info" text="**想定シーン**: 大学職員が、IR部門で使われているPythonのデータ処理スクリプトの仕組みを理解したいとき" />}}
...
{{< alert context="info" text="**想定シーン**: 大学職員が、動作はするがエラー処理のない既存のPythonスクリプトを堅牢なコードに改善したいとき" />}}
...
{{< alert context="info" text="**想定シーン**: 大学職員が、CSV出力時に発生したUnicodeEncodeErrorの原因を特定し、修正方法を知りたいとき" />}}
...
{{< alert context="info" text="**想定シーン**: 大学職員が、人事システムから出力されたJSON形式の職員データをCSVに変換して一覧表を作りたいとき" />}}
...
{{< alert context="info" text="**想定シーン**: 大学職員が、売上データの集計プログラムを作成する際に、分かりやすい変数名・クラス名のつけ方を参考にしたいとき" />}}
...
yearly_sales = sales_data[
    (sales_data['売上日'] >= start_date) &
    (sales_data['売上日'] <= end_date)
]

# 商品IDと売上日でグループ化し、数量を合計
grouped_summary = (yearly_sales
    .groupby(['商品ID', '売上日'])['数量']
    .sum()
    .reset_index())

# 売上日の昇順で並べ替え
sorted_summary = grouped_summary.sort_values('売上日')

# Excelファイルに出力
sorted_summary.to_excel('sales_summary.xlsx', index=False)
```

**命名のポイント**: `sales_data`（元データ）、`yearly_sales`（年次抽出）、`grouped_summary`（集計結果）、`sorted_summary`（並替済）のように、変数の内容が一目で分かる名前にしています。
{{% /alert %}}
...
{{< alert context="info" text="**想定シーン**: 大学職員が、pandasの主要メソッドの使い方を手軽に確認したいとき" />}}
...
{{< alert context="info" text="**想定シーン**: 大学職員が、授業評価アンケートやイベント参加申込のGoogle Formsの回答をGASで自動集計したいとき" />}}
...
{{< alert context="info" text="**想定シーン**: 大学職員が、フォルダ内の大量のファイルを命名規則に従ってリネーム・整理したいとき" />}}
