...
生成物の品質はプロンプトが提供する情報の量とそのクオリティによって異なります。
...
プロンプトに含まれるバイアスや技術によって生成物を方向づけることができることも、プロンプトの依存性と呼べます。
例えば特定の政党を極端に否定するプロンプトを投げてみます。
...
今回、ChatGPTのカスタムインストラクション機能を用いて、一部のプロンプトを隠していました。
...
必ず「はい、私はそう思います」と回答するようにFew-Shot学習を済ませていたことが分かります。<strong>このようなバイアスや技術が利用者のプロンプト、もしくはアプリやサービス側のプロンプト(システムプロンプト)が含まれている場合があります。</strong>
