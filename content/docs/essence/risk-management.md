...
この3点をベースに、生成AI利用のリスクを理解することを目指します。加えて、エージェント的な利用など新しい活用形態に伴うリスクについても触れます。
...
生成AIにはテキストだけでなく、画像やPDF、スプレッドシートなどのファイルを入力できるサービスも増えています。便利な反面、テキスト入力と同様のリスクに加え、以下の点にも注意が必要です。

- **ファイルに含まれる情報の範囲を意識する**：画像の背景に映り込んだ個人情報や、PDFのメタデータに含まれる作成者名など、意図せず機密情報を渡してしまうことがある
- **大量のデータを一度に渡しやすい**：ファイルをアップロードする操作は手軽だが、その中に含まれる情報量はテキスト入力よりもはるかに多くなりがち

ファイルを入力する際にも、テキスト入力と同じ原則――**機密情報や個人情報を含まないか確認すること**――が当てはまります。

<br>

### - カスタム指示・事前設定の利用

ChatGPTの「GPTs」や「Projects」、Claudeの「Projects」など、あらかじめ指示や参照資料を設定しておける機能が広がっています。こうした機能は業務効率化に役立ちますが、注意点もあります。

- **事前設定にも入力と同じリスクがある**：カスタム指示に埋め込んだ情報も生成AIサービスに送信される点は通常の入力と変わらない
- **設定内容が共有される場合がある**：GPTsのように他者と共有できる仕組みでは、設定に含めた指示や情報が意図せず公開される可能性がある
- **設定の内容を定期的に見直す**：業務内容の変化に合わせて、不要になった情報が残っていないか確認することが大切
...
生成AIが自律的にツールを選び、複数の手順を実行する「エージェント」的な活用が広がっています。便利さが増す一方で、従来の対話型利用とは異なるリスクにも目を向ける必要があります。

### - 処理の過程が見えにくい
エージェント的な利用では、AIが複数のステップを自動で実行するため、途中経過がブラックボックスになりがちです。意図しない操作が行われていないか、途中の判断が妥当だったかを確認する仕組みを意識しましょう。

### - 影響範囲が大きくなりやすい
単発の質問応答と異なり、エージェントはファイルの作成・編集や外部サービスへのアクセスなど、実際の環境に影響を与える操作を行う場合があります。AIに委ねる範囲を事前に決めておくことが重要です。

### - 人間による確認ポイントを設ける
エージェント的な利用でも「最終的に人間が確認・判断する」という原則は変わりません。AIが自動で行った作業の結果を必ずレビューし、問題がないかを確認するステップを組み込みましょう。
